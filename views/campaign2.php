	...
	$("#field-layer1_text").attr('maxlength','110');
		...
		$("#field-total_recipient").on("change",function()
		{
			...
			if(read[4]=="edit")
			{
				var id=read[5];
			  $.ajax({
			  		url: "<?php echo base_url(); ?>Campaign2/getConflict?primary_key="+id+"&start_time="+$("#field-start_time_campaign").val()+"&start_date="+date2[2]+"-"+date2[1]+"-"+date2[0]+"&end_time="+time,
						type: "GET",
						dataType: "text",
			...
			{
			  $.ajax({
			  		url: "<?php echo base_url(); ?>Campaign2/getConflict?start_time="+$("#field-start_time_campaign").val()+"&start_date="+date2[2]+"-"+date2[1]+"-"+date2[0]+"&end_time="+time,
						type: "GET",
						dataType: "text",
			  		success: function(response){
		...
		$("#field-start_date_campaign").on("change",function()
		{
			...
			if(new Date(parts[2]+"-"+parts[1]+"-"+parts[0])<=new Date())
			{
				$("#field-start_date_campaign").val("");
			}
				
			  $.ajax({
			  		url: "<?php echo base_url(); ?>Campaign2/getStartTime?id="+$("#field-start_date_campaign").val(),
						type: "GET",
						dataType: "text",
			  		success: function(response){
...
//			$("#iframe2").hide();
			var id=read[5];
			  $.ajax({
			  		url: "<?php echo base_url(); ?>Campaign2/getState?id="+id,
						type: "GET",
						dataType: "text",
			  		success: function(response){
			  			var json = $.parseJSON(response);
			  ...
			  $.ajax({
			  		url: "<?php echo base_url(); ?>Campaign2/getStartTime?primary_key="+id+"&id="+$("#field-start_date_campaign").val(),
						type: "GET",
						dataType: "text",
			  		success: function(response){
			  			var json = $.parseJSON(response);
